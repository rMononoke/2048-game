...
def run_game(grid_size_choice, theme_choice):
    ...
    HEIGHT = WIDTH + 120 # Чуть больше места для текста
    ...
    # [C.1 Undo Move] Список для хранения истории ходов
    history = [] 
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # [C.1 Undo Move] Логика отмены
                if event.key == pygame.K_u or event.key == pygame.K_BACKSPACE:
                    if len(history) > 0:
                        # Возвращаем последнее состояние
                        board, score = history.pop()
                        game_over = False # Если мы отменили проигрышный ход, игра продолжается

                if not game_over:
                    # Создаем глубокую копию ТЕКУЩЕГО состояния перед попыткой хода
                    # Это состояние "ДО ХОДА"
                    ...
                    if event.key == pygame.K_LEFT:
                        ...
                        board = init_board(GRID_SIZE)
                        score = 0
                        history = [] # Очищаем историю при рестарте
                    
                    # Если было движение и доска изменилась, сохраняем "старое" состояние в историю
                    if moved and prev_board != board:
                        history.append((prev_board, prev_score))
        ...
        screen.blit(high_text, (20, 45))
        
        # Подсказки управления
        ctrl_text = font_score.render("U - Undo | R - Restart | ESC - Menu", True, current_theme["text"])
        screen.blit(ctrl_text, (20, HEIGHT - 30))

        # Игровое поле
        start_y = 80
        pygame.draw.rect(screen, current_theme["board"], 
                         (GRID_PADDING, start_y, WIDTH - 2*GRID_PADDING, WIDTH - 2*GRID_PADDING - 40), # -40 для отступа снизу
                         border_radius=10)
        ...
        if game_over:
            ...
            screen.blit(over_txt, over_txt.get_rect(center=(WIDTH/2, HEIGHT/2 - 20)))
            
            # Подсказка про Undo при проигрыше
            undo_txt = font_score.render("Press U to Undo last move", True, WHITE)
            screen.blit(undo_txt, undo_txt.get_rect(center=(WIDTH/2, HEIGHT/2 + 30)))
            
            res_txt = font_score.render("Press R to Restart", True, (200, 200, 200))
            screen.blit(res_txt, res_txt.get_rect(center=(WIDTH/2, HEIGHT/2 + 60)))
...
def main_menu():
    ...
    while True:
        ...
        title = title_font.render("2048", True, theme_colors["text"])
        ...
        buttons = [
            ...
            Button(100, 350, 200, 60, "START", start, selected=True)
        ]
